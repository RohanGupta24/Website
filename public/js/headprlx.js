...
if ( /*window.mobilecheck() == false*/ 1) {
    var offset;
    var scrollEnd = 100;

    function blurImage(amount) {
        $('.splash-img-blur').css({
            //opacity: amount.toFixed(2)
        })
    }
    ...
    $(window).scroll(function () {
        if ($(window).scrollTop() == 0) {
            $(".splash").css({
                ...
                opacity: 1,
                '-webkit-animation': 'huehuehue 30s infinite linear'
            }, "slow");
        }
    });
    ...
    $(window).scroll(function () {
        ...
        if (scrollEnd / offset < 1.5 && scrollEnd / offset > 0) {
            ...
            o = +(((scrollEnd / offset) * 100) / 100).toFixed(1);
            $parallaxText.css({
                top: newPos.toFixed(5) + "px",
                opacity: o,
